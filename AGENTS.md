...
- `PYTHONPATH=src python3 -m stonewall --corpus hoss-stonewall/sample_corpus build --output build/reference` compiles the evidence reference.
- `PYTHONPATH=src python3 -m stonewall --corpus hoss-stonewall/sample_corpus verify --output build/reference` verifies byte parity.
- `PYTHONPATH=src python3 scripts/check_public_boundary.py` verifies the declared publication tree.
...
- There is no `package.json` or `requirements.txt`. The `pyproject.toml` package under `src/stonewall/` uses only the Python standard library. Other Python dependencies are injected at runtime via `uv run --with <pkg>`. Node scripts are zero-dependency ESM.
...
- `reference_data/reports/` is the output directory for generated reports — it's safe to regenerate at any time.
